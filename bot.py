        ...
        if len(row) == 0:
            ...
            if int(row) == 1:
                ...
                if days < 30:
                    if minutes >= 10:
                        bot.forward_message('@buyaccountgame', chat_id, message.message_id)
                        cur.execute("UPDATE members SET date = ? WHERE chat_id = ?", (now, chat_id))
                    else:
                        bot.send_message(chat_id, 'Для отправки сообщения подождите ещё ' + str(10 - minutes) + 'минут')
                else:
                    bot.send_message(chat_id, 'Ваша подписка закончилась((( Приобретите её снова командой /donate')
                    cur.execute("UPDATE members SET sub = ? WHERE chat_id = ?", (0, chat_id))
            else:
                hours = seconds // 3600
                if hours >= 1:
                    bot.forward_message('@buyaccountgame', chat_id, message.message_id)
                    ...
                    bot.send_message(chat_id, 'Для снятия ограничений на отправку приобретите премиум версию командой /donate')
